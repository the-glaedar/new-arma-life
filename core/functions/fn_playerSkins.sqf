...
switch (playerSide) do {
	...
	case east: {  //added 16/08
	if (LIFE_SETTINGS(getNumber,"civ_skins") isEqualTo 1) then {  
		if(uniform player == "U_C_Poloshirt_blue") then {  
			player setObjectTextureGlobal [0, "textures\Civil\Uniform\mignons.jpg"];  
		};  
		if(uniform player == "U_C_Poloshirt_burgundy") then {  
			player setObjectTextureGlobal [0, "textures\Civil\Uniform\fuck.jpg"];  
		};  
		if(uniform player == "U_C_Poloshirt_stripped") then {  
			player setObjectTextureGlobal [0, "textures\Civil\Uniform\che.jpg"];  
		};  
		if(uniform player == "U_C_Poloshirt_tricolour") then {  
			player setObjectTextureGlobal [0, "textures\Civil\Uniform\rebel.jpg"];  
		};  
		if(uniform player == "U_C_Poloshirt_salmon") then {  
			player setObjectTextureGlobal [0, "textures\Civil\Uniform\weed.jpg"];  
		};  
		if(uniform player == "U_C_Poloshirt_redwhite") then {  
			player setObjectTextureGlobal [0, "textures\Civil\Uniform\altis_life.jpg"];  
		};  
	};  
	};
};
