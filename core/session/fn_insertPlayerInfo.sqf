...
/*
    ...
    EDIT: It also gets sent to life_server\Functions\Custom\fn_insertStats.sqf
*/
...
switch (playerSide) do {
    ...
    case east: {
        _bank = LIFE_SETTINGS(getNumber,"bank_opfor");
    };
};
...
if (life_HC_isActive) then {
    ...
    diag_log "executing DB_fnc_insertRequest";
    [getPlayerUID player,profileName,CASH,_bank,player] remoteExecCall ["DB_fnc_insertRequest",RSERV];
    [getPlayerUID player,profileName,player] remoteExecCall ["DB_fnc_insertStats",RSERV]; //inserts the players levels into the `stats` table
};
