...
private ["_handle","_timeStamp","_extDB_notLoaded","_handle2"];
...
_extDB_notLoaded = "";
diag_log "----------------------------------------------------------------------------------------------------";
diag_log "------------------------------------ Starting New Arma Client Init ---------------------------------";
...
diag_log "::New Arma:: Data retrieved from `players` table";
waitUntil {life_session_completed_2};
...
switch (playerSide) do {
    ...
    case independent: {
  	...
  	case east: {
  		  life_paycheck = LIFE_SETTINGS(getNumber,"paycheck_opfor");
  	};
};
switch (playerSide) do {
    case west: {
    ...
    case independent: {
  	...
  	case east: {
  		  //Initialize opfor and blah  14/8
...
// Sets up the temporary variables for the session.
_handle2 = [] spawn life_fnc_setupTemp;
waitUntil {scriptDone _handle2};

diag_log "----------------------------------------------------------------------------------------------------";
diag_log format ["            End of New Arma Client Init :: Total Execution Time %1 seconds ",(diag_tickTime) - _timeStamp];
diag_log "----------------------------------------------------------------------------------------------------";
